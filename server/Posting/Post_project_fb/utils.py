...
# FILEPATH: c:/Users/Dell/Documents/Webappp/Web-application/server/Posting/Post_project_fb/utils.py

# ... existing code ...

def upload_linkedin_image(image_path: str):
    """Upload an image to LinkedIn and return the asset."""
    access_token = get_valid_linkedin_token()
    if not access_token:
        return {"status": "error", "message": "Missing or expired access token"}
    ...
    upload_url = upload_response_json['value']['uploadMechanism']['com.linkedin.digitalmedia.uploading.MediaUploadHttpRequest']['uploadUrl']
    asset = upload_response_json['value']['asset']

    # Upload the image file
    with open(image_path, 'rb') as image_file:
        image_data = image_file.read()

    upload_image_response = requests.put(upload_url, headers={'Authorization': f'Bearer {access_token}'}, data=image_data)

    if upload_image_response.status_code == 201:
        return asset
    return {"status": "error", "message": "Failed to upload image"}

def post_to_linkedin_with_image(message: str, image_path: str):
    """Post a message with an image to LinkedIn."""
    access_token = get_valid_linkedin_token()
    if not access_token:
        return {"status": "error", "message": "Missing or expired access token"}

    asset = upload_linkedin_image(image_path)
    if "status" in asset and asset["status"] == "error":
        return asset
